...
`max_ctx` is the alternative to a fixed `num_ctx`: instead of a size you guess up front, it's a VRAM ceiling, and the harness computes `num_ctx` per call from the actual prompt (via `agent_common/ollama.py`'s `estimate_num_ctx`/`_estimate_prompt_tokens`) — exact, using Ollama's own tokenizer over its (currently experimental, not-yet-shipped) `/api/tokenize` endpoint when available, otherwise a character-based estimate. The estimate is deliberately biased to overestimate: Ollama silently truncates a prompt that exceeds the loaded context (no error), so a bit of wasted VRAM is far cheaper than a critic silently reviewing truncated input. Prefer `max_ctx` for critics whose prompt size varies across a feature's lifetime (this is what prompted the feature — the `tasks` critic needed `num_ctx` raised by hand as `tasks.md` grew); keep explicit `num_ctx` for cases that need a fixed, reproducible footprint, like the eval fixtures in `tests/evals/common.py`. An explicit `num_ctx` always overrides `max_ctx` if both are set.
