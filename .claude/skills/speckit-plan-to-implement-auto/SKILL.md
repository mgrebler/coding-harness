...
description: Runs the full plan → tasks → test → implement pipeline for the current feature branch unattended, without stopping for review between stages. Chains plan-auto.py, tasks-auto.py, test-auto.py, and implement-auto.py sequentially. Resume-safe — re-running after any interruption continues from the first incomplete stage.
user-invocable: true
---
...
| Test | `specs/<feature>/test-critic-result-*.json` |
...
| Test | `specs/<feature>/test-critic-escalation.md` |
...
`/speckit-*-auto` skills, so you can review the artifact between stages,
