...
- `[TEST] <description>` — write failing test(s) for this behaviour only, no implementation
- `[IMPL] <description>` — implement to pass those tests (must reference paired [TEST] task ID)

No [IMPL] task may exist without a paired [TEST] task. No [TEST] task may exist without a paired [IMPL] task. Schema migrations and pure setup tasks that genuinely have no testable behaviour are the only exception — document the exception explicitly in the task entry itself.
...
3. **[TEST] or [IMPL] label**: REQUIRED for all behavioural tasks
   - `[TEST]` — this task writes failing test(s) only, no implementation
   - `[IMPL]` — this task writes implementation only, to pass the paired [TEST] task's tests
   - Setup/infrastructure tasks with no testable behaviour: NO type label (document exemption in description)
...
6. **Description**: Clear action with exact file path. [IMPL] tasks must reference their paired [TEST] task ID.
...
- **Phase 3+**: User Stories in priority order (P1, P2, P3...)
  - Within each story: [TEST] tasks (may be [P] relative to each other) → paired [IMPL] tasks
