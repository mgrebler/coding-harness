...
All orchestration logic lives in `.claude/agents/plan-auto.py`. This skill is a thin
...
```bash
python .claude/agents/plan-auto.py --feature 015-job-description-rich-text
```
...
6. On 3-iteration exhaustion: writes `specs/$FEATURE/plan-critic-escalation.md` and exits non-zero
