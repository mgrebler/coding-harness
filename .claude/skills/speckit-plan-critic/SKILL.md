...
```bash
python3 .claude/agents/plan_critic.py
```
...
- If no result file exists → write `specs/$ARGUMENTS/plan-critic-result-1.json`
- If `plan-critic-result-1.json` exists → write `plan-critic-result-2.json`
- If `plan-critic-result-2.json` exists → write `plan-critic-result-3.json`
...
```
[plan-critic] iteration 1 → FAIL (3 blocking, 1 warning) → specs/012-feature/plan-critic-result-1.json
```

or

```
[plan-critic] iteration 1 → PASS → specs/012-feature/plan-critic-result-1.json
```
