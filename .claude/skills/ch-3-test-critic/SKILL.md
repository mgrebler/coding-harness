...
- If no result file exists → write `$SPEC_DIR/ch-3-test-critic-result-1.json`
- If `ch-3-test-critic-result-1.json` exists → write `ch-3-test-critic-result-2.json`
- If `ch-3-test-critic-result-2.json` exists → write `ch-3-test-critic-result-3.json`
...
```
[ch-3-test-critic] iteration 1 → FAIL (3 blocking, 1 warning) → specs/015-feature/ch-3-test-critic-result-1.json
```

or

```
[ch-3-test-critic] iteration 1 → PASS → specs/015-feature/ch-3-test-critic-result-1.json
```
...
- Maximum 3 iterations. If tests have not passed after 3 runs, stop and write
  `$SPEC_DIR/ch-3-test-critic-escalation.md` with the full violation history from all attempts,
