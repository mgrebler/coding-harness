...
§TQ2 Red State Confirmed [BLOCKING]: a test-results/<TASKID>-red.txt artifact exists for every completed [TEST] task; artifact shows tests failing with assertion errors (e.g. "AssertionError: expected 404 to be 200") or module-not-found errors — an "AssertionError" line in the artifact IS a meaningful failure and PASSES §TQ2. §TQ2 fails when: (a) the artifact file is missing, (b) the artifact shows zero failing tests, (c) every failure is a syntax error in the test file itself, OR (d) the artifact shows only an infrastructure/environment failure — e.g. ECONNREFUSED, ENOTFOUND, ETIMEDOUT, "connection refused", "could not connect", "no tests found" — with no accompanying genuine assertion or module-not-found failure. Case (d) means the test environment didn't run, not that the test failed for the right reason — do not accept it as red-state confirmation even though it technically shows a failure.
